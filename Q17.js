function multiple_choice_matrix({ question_code, randomize, array_filter, exclusive_answers, hide_answers, validation, required } = {}) {
    ...
    document.querySelector("#p_next").addEventListener("click", (e) => {
        if ((array_filter !== undefined && array_filter["rows"] !== undefined && required === true) || (hide_answers !== undefined && hide_answers["rows"] !== undefined)) {
            var n_checked;
...
// Add the other brand into the placeholder if exists
let other = document.querySelector("#other");
if(other) {
  other.innerText = response.answers.find(answer => answer.questionCode == "Q6_99").value;
}