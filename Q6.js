function multiple_open_text({ question_code, array_filter, addons, numerical_validation, fill_empty_with, allow_empty_fields } = {}) {
  ...
  // Hide or show depending on input in Other textfield
  let other = document.querySelector("#p_Q6_99");
  let other_specify = document.querySelector("#q_Q6_99_card");
  let other_specify_box = other_specify.querySelector("input[type='text'], textarea");
  ...
  other.addEventListener("input", (e) => {
      if(Number(e.target.value) > 0){
        ...
        other_specify.style.display = "none";
        other_specify_box.value = "";
      }
  });
  
  document.querySelector("#p_next").addEventListener("click", (e) => {
    if(e.target.checked = true && other_specify.style.display == "block" && other_specify_box.value == "") {
      if(document.querySelector("#feedback_Q66_99") === null) {
        other_specify_box.classList.add("is-invalid");
        other_specify.scrollIntoView({ behavior: "smooth", block: "start", inline: "nearest" });
        other_specify_box.insertAdjacentHTML(
            "beforebegin",
            `<span class="d-block custom-error pb-1 text-center" id="feedback_Q66_99">
              <span class="form-error-message text-danger">Please, fill out the missing field</span>
            </span>`
        );
      }
